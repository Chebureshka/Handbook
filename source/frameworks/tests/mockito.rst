...
``Mockito.mock`` - оборачивает весь создаваемый объект, при этом вызов непереопределенных методов будет возвращать 0/null/пустые коллекции. |br|
...
``instanceof DataService`` вернёт true, ``dataServiceMock.getClass()`` — именно ``SomeInterface.class``

``Mockito.mockingDetails`` - возвращает ``MockingDetails``, содержащий информацию о замоканном объекте с точки зрения фреймворка Mockito
...
.. code-block:: java
    ...
    // будут возвращены методом последовательно, пока не вернётся последнее, после этого при последующих вызовах будет возвращаться только последнее
...
@Mock 
    Ставится перед полем и мокает указанного типа объект. |br|
